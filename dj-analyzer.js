...
let nativeDj = null;
try {
  nativeDj = require('./native/mineradio-bpm');
  if (!nativeDj || !nativeDj.isNativeAvailable || typeof nativeDj.djEnergyCreate !== 'function' || typeof nativeDj.buildBeatMapFromLowEnergy !== 'function') nativeDj = null;
} catch (e) {
  nativeDj = null;
}

function makeJsDjEnergyProcessor(sampleRate, hopSec) {
...
function openDjEnergyProcessor(sampleRate, hopSec) {
  if (nativeDj) {
    try {
      const handle = nativeDj.djEnergyCreate(sampleRate, hopSec);
      if (handle) {
        return {
          engine: 'cpp-native',
          pushStereo(left, right, n) {
            const L = left && n < left.length ? left.subarray(0, n) : left;
            const R = right ? (n < right.length ? right.subarray(0, n) : right) : null;
            nativeDj.djEnergyPushStereo(handle, L, R);
          },
          finish() {
            return nativeDj.djEnergyFinish(handle);
          },
        };
      }
    } catch (e) {}
  }
  return makeJsDjEnergyProcessor(sampleRate, hopSec);
}
...
function buildBeatMapFromLowEnergy(lowEnergy, hitEnergy, hopSec, durationSec) {
  if (nativeDj && typeof nativeDj.buildBeatMapFromLowEnergy === 'function') {
    try {
      // TypedArray 视图：JS 数组也能被 native 用? N-API 需要 TypedArray
      const low = (lowEnergy && lowEnergy.buffer) ? lowEnergy : Float32Array.from(lowEnergy || []);
      const hit = (hitEnergy && hitEnergy.buffer) ? hitEnergy : Float32Array.from(hitEnergy || []);
      const map = nativeDj.buildBeatMapFromLowEnergy(low, hit, hopSec, durationSec == null ? 0 : durationSec);
      if (map) return map;
    } catch (e) {
      // fall through
    }
  }
  return buildBeatMapFromLowEnergyJs(lowEnergy, hitEnergy, hopSec, durationSec);
}

function buildBeatMapFromLowEnergyJs(lowEnergy, hitEnergy, hopSec, durationSec) {
  ...
  const cameraBeats = beats.filter(b => !!b.camera);
  const pulseBeats = beats
    .filter(b => !!b.pulse && (b.impact >= 0.16 || b.combo === 'downbeat'))
...
async function decodePodcastDjEnergyRange(audioUrl, opts) {
  ...
  function processDecoded(result) {
    ...
    if (!energyProc) {
      energyProc = openDjEnergyProcessor(sr, hopSec);
      inputSr = sr;
    }
    ...
    decodedSamples += n;
    energyProc.pushStereo(left, right || null, n);
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (limitSec && inputSr && decodedSamples / inputSr >= limitSec) {
  ...
  const finished = energyProc ? energyProc.finish() : { lowEnergy: [], hitEnergy: [], effectiveSamples: 0, effectiveSampleRate: 0, sampleStep: 1, engine: 'none' };
  const lowEnergy = finished.lowEnergy || [];
  const hitEnergy = finished.hitEnergy || [];
  const effectiveSr = finished.effectiveSampleRate || 0;
  const effectiveSamples = Number(finished.effectiveSamples) || 0;
  const sampleStep = finished.sampleStep || 1;

  return {
    lowEnergy,
    hitEnergy,
    hopSec,
    duration: effectiveSr ? effectiveSamples / effectiveSr : durationHint,
    decode: {
      chunks,
      decodedSamples,
      sampleRate: effectiveSr ? effectiveSr * sampleStep : 0,
      effectiveSampleRate: effectiveSr,
      frames: lowEnergy.length || 0,
      engine: finished.engine || 'js',
    },
  };
}
...
async function analyzePodcastDjStreamFull(audioUrl, opts) {
  ...
  function processDecoded(result) {
    if (!result || !result.samplesDecoded || !result.channelData || !result.channelData.length) return;
    const sr = result.sampleRate || 44100;
    if (!energyProc) energyProc = openDjEnergyProcessor(sr, hopSec);
    const left = result.channelData[0];
    const right = result.channelData[1];
    const n = Math.min(result.samplesDecoded, left ? left.length : 0, right ? right.length : (left ? left.length : 0));
    decodedSamples += n;
...
module.exports = {
  ...
  buildBeatMapFromLowEnergyJs,
  nativeDjAvailable: !!nativeDj,
  openDjEnergyProcessor,
};
