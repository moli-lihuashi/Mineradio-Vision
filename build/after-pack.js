...
module.exports = async function afterPack(context) {
  ...
  // 阶段 2 DoD：打包后校验 DJ native addon（Electron ABI）是否随包
  var appDir = path.join(context.appOutDir, 'resources', 'app');
  var nativeNode = path.join(appDir, 'native', 'mineradio-bpm', 'build', 'Release', 'bpm_native.node');
  var djAnalyzer = path.join(appDir, 'dj-analyzer.js');
  if (!fs.existsSync(djAnalyzer)) {
    console.warn('  ⚠ afterPack: dj-analyzer.js missing — podcast DJ will be unavailable');
  }
  if (!fs.existsSync(nativeNode)) {
    console.warn('  ⚠ afterPack: bpm_native.node missing — DJ analysis will use JS fallback (slower)');
  } else {
    console.log('  • native DJ addon present:', nativeNode);
  }
};
