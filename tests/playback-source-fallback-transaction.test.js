...
const startPath = path.join(appRoot, 'public', 'js', 'modules', '05-playback', '09b-playback-core.js');
const stallPath = path.join(appRoot, 'public', 'js', 'modules', '05-playback', '09d-playback-stall-recovery.js');
...
const stallText = fs.readFileSync(stallPath, 'utf8');
...
function testStaticRecoveryWiring() {
  // 酷狗版：11-provider-fallback + 09b-playback-core + 09d-stall + 04-progress-seek
  assert(/beginSourceFallbackPlaybackInvocation\(opts\)/.test(startText));
  assert(
    /completeSourceFallbackRecovery\(opts\.sourceFallbackRecovery\)/.test(startText)
    || /completeSourceFallbackRecovery\(sourceFallbackRecoveryFromOptions\(opts\)\)/.test(startText)
  );
  assert(/tryAutoPlaybackFallback\(/.test(startText));
  assert(/skipFailedQueueItem\(/.test(startText));
  assert(/freshUrlAttemptCount\) \|\| 0\) >= 1/.test(stallText));
  assert(/return recovered === true/.test(stallText) || /if \(recovered === true\) return true/.test(stallText));
  ...
  assert(/function playbackStallRecoveryOwnerStillCurrent/.test(stallText));
  assert((stallText.match(/playbackStallRecoveryOwnerStillCurrent\(/g) || []).length >= 2);
  assert(/recoverySerial !== playbackResumeRecovery\.serial/.test(stallText));
  assert(/\['play', 'playing', 'pause'[\s\S]{0,500}audioEl !== audio/.test(progressText));
  assert(/\['waiting', 'stalled', 'error'\][\s\S]{0,700}schedulePlaybackStallRecovery/.test(progressText));
  assert((startText.match(/else setTimeout\(nextTrack, 0\)/g) || []).length >= 1, 'normal ended playback must still advance');
  const nextTrackBlock = startText.slice(
    startText.indexOf('function nextTrack'),
    startText.indexOf('function prevTrack')
  );
  assert(/playQueueAt\(currentIdx,\s*\{/.test(nextTrackBlock));
