(function () {
  ...
  var hideUiActive = true;
  var hideUiTimer = null;

  function scheduleHideUiReapply() {
    if (hideUiTimer) return;
    hideUiTimer = setInterval(function () {
      if (!hideUiActive) {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (data.hideUi != null) hideUiActive = !!data.hideUi;

    if (data.properties) {
      var merged = Object.assign({}, embeddedDefaults, data.properties);
      merged.showAlbumCover = { value: false };
      merged.showPlayerController = { value: false };
      applyBridgeProperties(merged);
      if (hideUiActive) scheduleHideUiReapply();
    }
  });
})();
