...
function beatMapSongKey(song) {
  ...
  if (songProviderKey(song) === 'qishui') return 'qishui:' + (song.id || song.providerSongId || (song.name + '|' + song.artist));
...
async function fetchBeatPrefetchAudioUrl(song) {
  ...
  var data = typeof resolveProviderPlayUrlData === 'function'
    ? await resolveProviderPlayUrlData(song, {})
    : null;
  if (!data || !data.url || data.trial) return null;
  return await Mineradio.api.apiMediaProxyUrlAsync('/api/audio', { url: data.url });
}
...
function scheduleQueueBeatPrefetch(fromIdx, delayMs, state) {
  ...
  var prefetchLimit = typeof effectiveBeatPrefetchLimit === 'function' ? effectiveBeatPrefetchLimit() : BEAT_PREFETCH_LIMIT;
  if (prefetchLimit <= 0) return;
  var prefetchState = normalizeBeatPrefetchState(state);
  if (prefetchState.count >= prefetchLimit) return;
...
async function runQueueBeatPrefetch(fromIdx, token, seq, state) {
  ...
  if (state.count >= prefetchLimit) return;
  ...
  try {
    ...
    if (state.count < prefetchLimit && token === beatMapToken && seq === beatPrefetchToken && playQueue.length && !(audio && audio.paused)) {
