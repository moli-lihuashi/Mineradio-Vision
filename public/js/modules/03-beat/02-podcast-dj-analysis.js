...
async function analyzePodcastDjIntroBeats(audioUrl, token, durationSec) {
  ...
  var introData = await Mineradio.api.apiJson('/api/podcast/dj-beatmap?url=' + encodeURIComponent(audioUrl) + '&duration=' + encodeURIComponent(durationSec || 0) + '&intro=180').catch(function(){ return null; });
  if (token !== djBeatMapToken || !djMode.active) return null;
...
async function buildPodcastDjLowOnlyBeatMap(buffer, token) {
  ...
  var pulseBeats = beats.filter(function(b){ return !!b.pulse && (b.impact >= 0.16 || b.combo === 'downbeat'); }).map(function(b){
...
async function analyzePodcastDjBeats(audioUrl, token, durationSec) {
  try {
    ...
    if (preferServerAnalysis) {
      ...
      var serverData = await Mineradio.api.apiJson('/api/podcast/dj-beatmap?url=' + encodeURIComponent(audioUrl) + '&duration=' + encodeURIComponent(durationSec)).catch(function(){ return null; });
      if (token !== djBeatMapToken || !djMode.active) { hideBeatChip(); return null; }
    ...
    var fetchAudioUrl = /^https?:\/\//i.test(audioUrl || '')
      ? await Mineradio.api.apiMediaProxyUrlAsync('/api/audio', { url: audioUrl })
      : audioUrl;
    var resp = await (Mineradio.api.apiFetch || fetch)(fetchAudioUrl);
    if (token !== djBeatMapToken || !djMode.active) { hideBeatChip(); return null; }
