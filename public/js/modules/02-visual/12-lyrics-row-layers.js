...
function makeLyricBackfaceReadableMaterial(opts) {
  ...
  // 兼容旧路径（14-stage-lyrics-rendering tickMesh 等）对 mat.opacity / mat.color / mat.map 的直接读写。
  // 本函数会覆盖 10-lyrics-mask-shaders.js 的同名实现，若缺少这些代理属性，
  // legacy tick 里 data.glowMat.color.copy(...) 会每帧抛 TypeError 并中断 animate 主循环，
  // 导致 renderer.render 永不执行（画面冻结、歌词粒子全部消失）。
...
function makeLyricRowLayerGroup(payload, mask, worldW, worldH, pal, motionProfile) {
  ...
  // 同步路径用 Infinity。直播大轨由 showStageLine → cooperative 分帧，避免走此同步死循环。
  while (!stepLyricRowLayerGroupBuild(state, 1024, Infinity)) { /* drain phases */ }
...
function resetLyricRenderUploadFrameBudget(deferQualityFinalize) {
  ...
  // P3：默认 1；首屏露字由 updateLyricRowLayers 临时抬到 3，保证活跃行当帧上传
...
function consumeLyricRenderUploadFrameBudget(force) {
  if (force === true) {
...
function updateLyricRowLayers(data, opts) {
  ...
  if (initialTextRevealPending) {
    // P3：首屏露字 — 活跃行/译文当帧强制上传，不占满预算拖死 WebGL/预设
    lyricRenderUploadFrameBudget.remaining = Math.max(lyricRenderUploadFrameBudget.remaining, 3);
  }
  if (renderRevealCandidates.length) {
    ...
    for (var revealIndex = 0; revealIndex < renderRevealCandidates.length; revealIndex++) {
      ...
      var forceActiveUpload = !!(initialTextRevealPending && Number(reveal.priority) <= 1 && !reveal.transparentPrewarm && !reveal.quality);
      if (!consumeLyricRenderUploadFrameBudget(forceActiveUpload)) break;
      if (reveal.quality) {
