/* ===== 内存管理 UI（适配 configureMemoryAuto）===== */
...
function refreshMemorySnapshot(force) {
  if (!window.desktopWindow || typeof desktopWindow.getMemorySnapshot !== 'function') {
  ...
  return desktopWindow.getMemorySnapshot().then(function(snapshot) {
    var status = snapshot ? memoryFormatSnapshot(snapshot) : '系统内存读取失败';
...
function toggleMemoryMaskPart(part) {
  ...
  saveLyricLayout();
  updateMemoryControls();
  configureMemoryAutoFromFx(false);
}
function configureMemoryAutoFromFx(runNow) {
  if (!window.desktopWindow || typeof desktopWindow.configureMemoryAuto !== 'function') return Promise.resolve(null);
  return desktopWindow.configureMemoryAuto(memoryAutoConfigPayload(runNow)).then(function() {
    updateMemoryControls();
    return null;
  }).catch(function(error) {
...
function runAppMemoryTrim(reason) {
  if (!window.desktopWindow || typeof desktopWindow.trimAppMemory !== 'function') {
    showToast('桌面版才支持进程内存压缩');
    return;
  }
  updateMemoryStatusText('正在压缩播放器工作集...');
  desktopWindow.trimAppMemory([null]).then(function() {
    return refreshMemorySnapshot(true);
  }).then(function() {
    showToast('播放器工作集已压缩');
  }).catch(function(error) {
...
function runSystemMemoryPurge(autoElevate) {
  ensureMemoryFxDefaults();
  if (!window.desktopWindow || typeof desktopWindow.purgeSystemMemory !== 'function') {
    showToast('桌面版才支持系统级释放');
    return;
  }
  updateMemoryStatusText(autoElevate ? '正在请求提权系统释放...' : '正在尝试系统级手动释放...');
  desktopWindow.purgeSystemMemory({
    mask: normalizeMemorySystemMask(fx && fx.memorySystemMask),
    ...
    if (result && result.ok) {
      ...
      updateMemoryStatusText(autoElevate ? '提权释放未完成：可能取消了管理员权限' : '当前权限只能完成部分释放；需要时可点提权释放');
      showToast(autoElevate ? '提权释放未完成' : '需要管理员权限的部分已跳过');
    } else {
      updateMemoryStatusText('系统释放未完成: ' + String(result && result.message || ''));
    }
