/*! morphicons 1.7.1 — MIT License — https://github.com/guillermolg00/morphicons
 *  Classic-script build for Mineradio (no bundler / no ESM):
 *  merged from dist/normalize-CYnN3Npw.js + dist/spring-CFHloqPP.js + dist/dom.js
 *  (upstream ESM sources, import/export statements removed, IIFE-wrapped).
 *  Exposes window.Morphicons = { createMorph, canonicalD, fitIcon }.
 *  createMorph(pathEl, iconD) → { morphTo(iconD, spring?), set(iconD), destroy() }
 *  Icons are raw SVG path `d` strings on a 24×24 grid (Lucide-style stroke icons).
 */
(function (global) {
  'use strict';

  // ===== upstream: dist/normalize-CYnN3Npw.js =====
  ...
  *  resulting `d` anywhere an icon is accepted. */
  ...
  // ===== upstream: dist/spring-CFHloqPP.js =====
