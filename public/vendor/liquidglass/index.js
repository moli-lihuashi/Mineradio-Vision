...
var BLUR_ITERATIONS = 4; // Optimized from 6 for performance
var SHADOW_PAD = 20;
...
function getPixelRatio() {
  ...
  return Math.min(ratio || window.devicePixelRatio || 1, 2);
}
...
var LiquidGlass = class _LiquidGlass {
  ...
  _renderLoop() {
    ...
    // 页面不可见时彻底停止调度，避免每帧 offsetWidth/offsetHeight 触发 reflow
    // 由 visibilitychange 回调恢复
  ...
  _renderGlassElement(child, rootRect, dpr, isDragging, dirtyTargets, renderedThisFrame) {
    ...
    if (needsShaderRender && glassCanvas) {
      ...
      // FBO 缓存上限 6 个，超过释放最旧
      if (this._glassCache.size > 6) {
        const firstKey = this._glassCache.keys().next().value;
        this._glassCache.delete(firstKey);
      }
  ...
  _composeSceneForGlass(currentGlass, sampleRect, rootRect, dpr) {
    ...
    this._drawPageBackdropToScene(sampleRect, rootRect, dpr);
    for (const child of this._sortedChildren) {
  ...
  /**
   * Draw the live page backdrop (main WebGL scene mirror, kept fresh by the
   * app's main render loop) into the local scene canvas so glass refracts the
   * real background instead of a flat fill. Falls back to the flat base fill
   * when no mirror is available.
   */
  _drawPageBackdropToScene(sampleRect, rootRect, dpr) {
    const mirror = window.__lgBackdropCanvas;
    const info = window.__lgBackdropRect;
    if (!mirror || mirror.width <= 0 || mirror.height <= 0 || !info || !info.scale) return;
    const k = info.scale / dpr;
    const srcX = Math.round((rootRect.left - info.left) * info.scale + sampleRect.x * k);
    const srcY = Math.round((rootRect.top - info.top) * info.scale + sampleRect.y * k);
    const srcW = Math.max(1, Math.round(sampleRect.w * k));
    const srcH = Math.max(1, Math.round(sampleRect.h * k));
    if (srcX + srcW <= 0 || srcY + srcH <= 0 || srcX >= mirror.width || srcY >= mirror.height) return;
    try {
      this._sceneCtx.drawImage(
        mirror,
        srcX, srcY, srcW, srcH,
        0, 0, sampleRect.w, sampleRect.h
      );
    } catch (_e) {
    }
  }
  _prepareSceneCanvas(width, height) {
    ...
    this._sceneCtx.fillStyle = "#0a0d12";
...
if (typeof window !== 'undefined') {
  ...
  window.LiquidGlassDEFAULTS = DEFAULTS;
  window.LiquidGlassInvalidateFontEmbedCache = invalidateFontEmbedCache;
}
